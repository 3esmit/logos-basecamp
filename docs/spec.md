...
QML-based UI Apps (`ui_qml` modules) load directly into the Basecamp process, so an untrusted app is confined by a sandbox applied to its QML engine (`QmlSandbox::configure`). A `ui_qml` app is QML/JS only and must stay inside its own install directory; the sandbox enforces this on four fronts:
...
- **User-selected files:** each app view receives a `userFiles` context object. `openFile(nameFilters, maxBytes)` returns an opaque request ID immediately and opens a host-owned native picker without a nested event loop. `fileSelectionCompleted(requestId, selection)` later delivers `{ handle, displayName, byteLength }`, or an empty selection for cancellation/rejection. The selected regular file is copied into host memory before completion; QML reads ordered chunks through `readNextChunk(handle)` as `{ base64, sequence, eof }` and revokes the handle with `release(handle)`. Paths never enter QML. Handles are isolated per view and capped at four handles / 40 MiB, files at 10 MiB, and chunks at 32 KiB. Destroying the view closes its pending picker, suppresses completion, and revokes every handle.
