...
- **User-selected files:** each app view receives a `userFiles` context object. `openFile(nameFilters, maxBytes)` uses a host-owned native picker and returns only `{ handle, displayName, byteLength }`. The selected regular file is copied into host memory immediately; QML reads ordered chunks through `readNextChunk(handle)` as `{ base64, sequence, eof }` and revokes the handle with `release(handle)`. Paths never enter QML. Handles are isolated per view and capped at four handles / 40 MiB, files at 10 MiB, and chunks at 32 KiB.
