...
│   │   └── UserSelectedFileBridge.h/cpp     # opaque file capabilities
...
**Files:** `src/restricted/QmlSandbox.h/cpp`, `src/restricted/DenyAllNetworkAccessManager.h/cpp`, `src/restricted/DenyAllNAMFactory.h/cpp`, `src/restricted/DenyAllReply.h/cpp`, `src/restricted/RestrictedUrlInterceptor.h/cpp`, `src/restricted/UserSelectedFileBridge.h/cpp`

**Purpose:** Security layer for QML-based UI Apps (`ui_qml` modules), applied by `QmlSandbox::configure()` (the single setup `PluginLoader::loadQmlView` runs on each app's `QQmlEngine`). A `ui_qml` app is meant to be QML/JS only, confined to its own install directory; the sandbox enforces that on four fronts:
...
- **User-selected files:** a per-view `UserSelectedFileBridge` is injected as `userFiles`. `openFile()` returns an opaque request ID immediately and uses `QFileDialog::open()` so no nested event loop can outlive the view. `fileSelectionCompleted()` delivers a snapshotted regular, non-symlink file as an opaque handle, display name, and length; cancellation or rejection delivers an empty selection. Stateful 32 KiB base64 chunk reads prevent path forgery and preserve ordering. View destruction closes and deletes the pending dialog, suppresses callbacks, and revokes all handles. Limits are 10 MiB per file, four handles / 40 MiB per view, and one active picker.
...
| `UserSelectedFileBridge` | Per-view async capability bridge for native-picker selections; snapshots bytes, enforces limits, streams canonical base64 chunks, cancels with view destruction, and never exposes a host path |
...
User clicks "Load" in UI Apps tab (or clicks app icon in sidebar)
 └─ MainUIBackend::loadUiModule(name)
     ...
     ├─ Create UserSelectedFileBridge → inject as "userFiles" context property
